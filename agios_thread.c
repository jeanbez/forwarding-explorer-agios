...
{
	g_agios_thread_stop = true; //we chose not to protect this variable with a mutex because there is a single thread writing to it and a single thread reading it, the worse it could happen is that the agios thread does not read correctly, but then it will read it later. It is not a big deal if the agios_exit call waits a little longer.
...
void * agios_thread(void *arg)
{
	...
	struct io_scheduler_instance_t *dynamic_scheduler=NULL; /**< The scheduling algorithm chosen in the configuration parameters. */ 
	...
	uint32_t remaining_time; /**< Used to calculate how long until we change the scheduling algorithm again */
	...
	if (!dynamic_scheduler->is_dynamic) { //we are NOT using a dynamic scheduling algorithm
		...
		agios_gettime(&last_algorithm_update);	//we will change the algorithm periodically		
	}
	...
	do {
		...
		if (!g_agios_thread_stop) {
			...
			if (dynamic_scheduler->is_dynamic) {
				remaining_time = get_nanoelapsed(last_algorithm_update);
				if (remaining_time >= contig_agios_select_algorithm_period) { //it is time to select!
			...
			if (current_reqnb > 0) { //it could be outdated info since we are not using the lock
				remaining_time = current_scheduler->schedule(client);
				if (remaining_time < current_timeout) current_timeout = remaining_time;
			}
		}
