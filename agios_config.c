...
static short int config_select_algorithm = 0;
...
{
	config_select_algorithm = value;
}
inline short int config_get_select_algorithm(void)
{
	return config_select_algorithm;
}
...
inline short int read_configuration_file(char *config_file)
{
	...
	config_lookup_bool(&agios_config, "library_options.select_algorithm", &ret);
	config_set_select_algorithm(ret);
...
void config_print(void)
{
	...
	config_print_flag(config_select_algorithm, "Will AGIOS try to select the best scheduling algorithm automatically? ");
